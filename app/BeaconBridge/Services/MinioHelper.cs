...
public class MinioHelper(ILogger logger, MinioOptions minioSettings) : IMinioHelper
{
  ...
  public async Task<bool> CreateBucketPolicy(string bucketName)
  {
    ...
    var signer = new AWS4RequestSigner(minioSettings.AccessKey, minioSettings.SecretKey);
    ...
    var request = new HttpRequestMessage
    {
      ...
      RequestUri = new Uri(minioSettings.Url + "/minio/admin/v3/add-canned-policy?name=" + bucketName + "_policy"),
      Content = content
    };

    request = await signer.Sign(request, minioSettings.AWSService, minioSettings.AWSRegion);
...
{{
  ...
  private AmazonS3Config GenerateAmazonS3Config()
  {
    var config = new AmazonS3Config
    {
      ...
      ServiceURL = minioSettings.Url, // replace http://localhost:9000 with URL of your MinIO server
      ForcePathStyle = true, // MUST be true to work correctly with MinIO server
    };

    if (minioSettings.UesProxy)
    {
      config.SetWebProxy(new System.Net.WebProxy
      {
        Address = new Uri(minioSettings.ProxyAddresURL),
        BypassList = new[] { minioSettings.BypassProxy }
      });
    }
